...
import type { PlaceCandidate, DiscoveryProfile } from "../src/shared/types.js";
...
async function querySql(sql: string): Promise<unknown[]> {
  ...
  // exec_sql won't exist; use direct table queries instead
...
async function showRun(runId: string): Promise<void> {
  ...
  const { data: run } = await db
    .from("runs")
    .select("id, status, niche, location, profile, discovered, filtered, created_new, updated_existing, completed_at")
    .eq("id", runId)
    .single();
...
async function showLeads(runId: string): Promise<void> {
  ...
  const { data: leads } = await db
    .from("leads")
    .select("place_id, name, rating, user_rating_count, website_uri, first_seen_run_id, state, tags")
...
async function main(): Promise<void> {
  const profile: DiscoveryProfile = "a";
  ...
  console.log("=".repeat(60));
  console.log(`Profile: ${profile} | Niche: ${niche} | Location: ${location}`);
  console.log(`Fixture candidates: ${FAKE_PLACES.length}`);

  // ---- RUN 1 ---------------------------------------------------------------
  console.log("\n--- RUN 1 (first discovery, should create new leads) ---");

  const run1 = await createRun({ niche, location, profile, maxResults: 10, minRating: 4.0 });
  console.log(`Run created: ${run1.id}  status=${run1.status}`);

  const filtered1 = applyProfileFilter(FAKE_PLACES, profile, 4.0);
  console.log(`Filtered: ${FAKE_PLACES.length} → ${filtered1.length} passed profile A`);
  ...
  const { inserted: ins1, updated: upd1 } = await upsertLeads(
    filtered1, run1.id, profile, (c) => tagCandidate(c, profile)
  );
  ...
  console.log(`\nInserted: ${ins1.length}  Updated: ${upd1.length}  duration_ms: ${dur1}`);
  ...
  const run2 = await createRun({ niche, location, profile, maxResults: 10, minRating: 4.0 });
  console.log(`Run created: ${run2.id}  status=${run2.status}`);

  const filtered2 = applyProfileFilter(FAKE_PLACES, profile, 4.0);
  const { inserted: ins2, updated: upd2 } = await upsertLeads(
    filtered2, run2.id, profile, (c) => tagCandidate(c, profile)
  );
  ...
  console.log("→ Expected: inserted=0, updated=4 (all already existed)");
  ...
  const { data: dedupCheck } = await db
    ...
    .in("place_id", filtered1.map((c) => c.placeId));
  ...
  const run3 = await createRun({ niche, location, profile, maxResults: 10, minRating: 4.0 });
  ...
  const { data: failedRun } = await db.from("runs").select("id, status, error").eq("id", run3.id).single();
  console.log(`\nFailed run: status=${failedRun?.status}  error="${failedRun?.error}"`);
  console.log(failedRun?.status === "failed" ? "✅ status=failed" : "❌ status should be failed");

  // ---- Final summary -------------------------------------------------------
  const { data: allRuns } = await db.from("runs").select("id, status, created_new, updated_existing").order("created_at");
  console.log("\n=== all runs (final state) ===");
  console.table(allRuns);

  const { data: allLeads } = await db.from("leads").select("place_id, name, rating, state, tags, first_seen_run_id").order("rating", { ascending: false });
