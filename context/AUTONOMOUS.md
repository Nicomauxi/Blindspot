...
| `approval-required-fase-30-research` | Item 43 (Fase 30 DGI) requiere Gemini DeepSearch del Tech Lead validando fuente DGI/BPS oficial. **El research YA FUE COMPLETADO y está en `context/research/dgi.md`.** Leer ese archivo, evaluar si la fuente es viable y proceder con Fase 30 si lo es; si no hay fuente viable concluir con `fases-completas-con-research-pendiente`. | Leer `context/research/dgi.md` y decidir si la fuente es implementable antes de arrancar. |
...
| `fases-completas-con-research-pendiente` | Items 0–41 completos. Item 42 (Fase 42 scoring estacional): sin data de conversión suficiente. Item 43 (Fase 30 DGI): research completado en `context/research/dgi.md` — leerlo y continuar si la fuente es viable, o declarar `fases-completas` si no lo es. | Leer `context/research/dgi.md` y continuar con Fase 30, o cerrar el roadmap. |
...
**Stop condition activa:** `fases-completas-con-research-pendiente` — Items 0–41 completos. Item 42 (scoring estacional): sin data de conversión suficiente todavía. Item 43 (Fase 30 DGI): **research en `context/research/dgi.md` — leer ese archivo en el Paso 4 antes de implementar**. Si la fuente es viable → implementar Fase 30. Si no → stop con `fases-completas`.
